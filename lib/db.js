...
export async function upsertConversation(convo) {
  ...
  if (existing && convo.pinned === undefined) {
...
// Internal: put a folder record with its id preserved (used by backup import).
export async function _putFolderRaw(folder) {
  const db = await openDb();
  return new Promise((resolve, reject) => {
    const req = tx(db, "readwrite", FOLDERS).put(folder);
    req.onsuccess = () => resolve(folder);
...
export async function togglePinned(key) {
  const db = await openDb();
  return new Promise((resolve, reject) => {
    ...
    get.onsuccess = () => {
      ...
      put.onsuccess = () => resolve(convo.pinned);
      put.onerror = () => reject(put.error);
    };
    get.onerror = () => reject(get.error);
  });
